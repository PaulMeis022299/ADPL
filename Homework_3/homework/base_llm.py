...
class BaseLLM:
    ...
    def batched_generate(
        ...
        # decode the outputs with self.tokenizer.decode
        generated_tokens = outputs[:, len(inputs["input_ids"][0]) :]
        decoded = self.tokenizer.batch_decode(generated_tokens, skip_special_tokens=True)

        if not num_return_sequences:
            output = [d.strip() for d in decoded]
            #print(prompts)
            #print(output)
            return output
        else:
            output = [
                [decoded[i * num_return_sequences + j].strip() for j in range(num_return_sequences)]
